...
<template>
  <div class="flex w-full items-start gap-2">
    <Button
      variant="outline"
      size="icon"
      class="mt-px size-10 shrink-0"
      as-child
    >
      <label class="relative flex size-10 cursor-pointer items-center justify-center overflow-hidden rounded-md">
        <input
          type="file"
    ...
    <InputGroup :class="cn('min-h-16 min-w-0 flex-1')">
      <InputGroupTextarea
        v-model="prompt"
      ...
      <InputGroupAddon
        ...
        :class="
          cn(
            'mt-px flex flex-row flex-wrap items-center gap-2 border-t border-border px-3 py-3 justify-between!',
          )
        "
      >
        <div class="flex min-w-0 items-center gap-2">
          <span class="shrink-0 text-xs font-medium whitespace-nowrap text-muted-foreground">
            采集页数
          </span>
